...
Object.entries(antennas).forEach(([_antenna, positions]) => {
  positions.forEach((position, positionIndex) => {
    antinodes.add(`${position.x},${position.y}`);
    ...
    for (let i = 0; i < positions.length; i++) {
      ...
      while (!(antidodePoint.x >= width || antidodePoint.x < 0 || antidodePoint.y >= height || antidodePoint.y < 0)) {
        const antidodePointString = `${antidodePoint.x},${antidodePoint.y}`;

        antinodes.add(antidodePointString);

        antidodePoint.x += deltaX;
        antidodePoint.y += deltaY;
      }
    }
  });
});
